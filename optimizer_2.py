...
# Die Aufgabe die wir optimieren
...
def main():
    ...
    for idx, variant in enumerate(PROMPT_VARIANTS, 1):
        ...
        word_count = len(output.split()) # Counts the words within the output
        ...
        # Manuel scoring for Bronze
        print(f"\n{CRITERIA}")
        scores = get_manual_scores()

        # Calculate total score
        content_score = (scores["empathy"] + scores["professionalism"] + scores["concreteness"]) / 3 # Calculate average score from 3 subscores
        word_count_weight = word_count_factor(word_count)
        final_score = round(content_score * word_count_weight, 2) # Apply wordcount weight
        ...
        results.append({
            ...
            "total_score": final_score, # average score
